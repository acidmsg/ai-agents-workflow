...
1. Создай директорию `tmp/` (если её нет) и запусти проверку markdownlint на всех .md файлах:

    ```powershell
    New-Item -Force -ItemType Directory tmp > $null ; npx markdownlint "specs/**/*.md" ".roo/**/*.md" "*.md" > tmp/.tmp_mdlint.txt 2>&1
    ```

    Прочитай `tmp/.tmp_mdlint.txt`. Если есть ошибки — покажи их пользователю, удали `tmp/.tmp_mdlint.txt` и остановись (commit не выполняй).

2. Если markdownlint прошёл чисто — удали `tmp/.tmp_mdlint.txt` и продолжай.
